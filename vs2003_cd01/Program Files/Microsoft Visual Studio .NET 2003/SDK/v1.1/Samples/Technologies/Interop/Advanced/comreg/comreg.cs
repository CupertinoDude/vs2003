			...
			try
			{
				...
				else
				{
					...
					if (gac)
					{
						if(unregister)
						{
							Console.WriteLine("Warning uninstall currently only removes the file from the GAC");
					...
					if (exitcode == 0)
					{
						...
						if(unregister)
						{
							Console.WriteLine("Assembly: " + assemblyfile + " unregistered" + ((gac) ? " and removed from the GAC." : "."));
						}
						else
						{
							RegisterAssembly(assemblyfile, regfile, psmarshaler, codebase, remotehost, runas);
							Console.WriteLine("Assembly: " + assemblyfile + " registered" + ((gac) ? " and added to the GAC." : "."));
						}
					}
				}
			}
