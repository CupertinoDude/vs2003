...
namespace ADepends
   ...
   * aspects.  The reason is that a SerializationException is
   ...
   * The alternative would be to do something less elegant, like:
   *
   ...
   * This isn't possible when an object exposing the IEnumerable interface
   * is transferred between AppDomains, we go through COM Interop via a 
   * Transparent Proxy.
   * However, the interop code doesn't check for a Transparent Proxy, 
   * and instead asserts that we're a COM object...which we're not.
   * So the ASSERT fires.
   *
