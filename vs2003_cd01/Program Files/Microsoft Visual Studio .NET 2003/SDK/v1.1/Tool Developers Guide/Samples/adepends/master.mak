...
_LFLAGS=$(_L_DBG_FLAGS)
_LDFLAGS=-dll $(_L_DBG_FLAGS)
...
_VB_DLL_FLAGS=/t:library $(_VB_DBG_FLAGS)
_VB_EXE_FLAGS=/t:exe $(_VB_DBG_FLAGS)
...
.vb.dll:
	if not exist $(_OUTDIR) md $(_OUTDIR)
	$(_BC) $(_VB_DLL_FLAGS) $(_IMPORTS) /out:$(_OUTDIR)\$@ $*.vb

.vb.exe:
	if not exist $(_OUTDIR) md $(_OUTDIR)
	$(_BC) $(_VB_EXE_FLAGS) $(_IMPORTS)  /out:$(_OUTDIR)\$@ $*.vb
