...
void BASEHELPER::DDebug( char *format, ... )
{
    ...
    {
    	va_list args;
    	char buffer[1024];	// largest possible for wvsprintf
		...
		default:
			{
				...
				static char logfile[MAX_LENGTH+MAX_PATH];

				
				if ( count++ == 0 )
				{
					InitializeCriticalSection( &criticalSection );
					if ( GetEnvironmentVariableA( LOG_OUTPUT_FILE, logfile, MAX_LENGTH+MAX_PATH ) == 0 )
					{
						//
						// if var does not exist, use the defaul directory and the default name
						//
                ...
                
				{
					...
					//
					stream = ((count == 1) ? fopen( logfile, "w" ) : fopen( logfile, "a+" ));
				    if ( stream != NULL )
				    {
...
DECLSPEC
/* static public */
void BASEHELPER::PrintTheWin32Error( DWORD dwError )
{
	LPVOID lpMsgBuf;
	static DWORD launchWindow = BASEHELPER::FetchEnvironment( DEBUG_ENVIRONMENT );

	
	if ( (launchWindow >= 1) && (launchWindow != 0xFFFFFFFF) )
    {    	
	...
	{
		char strMessage[MAX_LENGTH];

		sprintf( strMessage, "GetLastError() Returned Error Code: 08%x", dwError );
		TEXT_OUTLN( strMessage )
	}

} // BASEHELPER::PrintTheWin32Error
 ...
 ***************************************************************************************/
/* public */
char *BASEHELPER::GetEERegistryKeySTRING( char *regKeyName )
{
	DWORD type;
    DWORD size;
    HKEY userKey = NULL;
    ...
    if ( (RegOpenKeyExA( HKEY_LOCAL_MACHINE, 
          ...
          (type == REG_SZ) )
   	{                	        
   		retValue = new char[size + 1];
   		sprintf( retValue, "%s", buffer );  
   		      
		printf( "Registry LM Variable: %s=%s\n", regKeyName, retValue );
	}
    //
    // current user
    //
   	...
   	{                	                    
		...
		printf( "Registry CU Variable: %s=%s\n", regKeyName, retValue );	
	}
...
} // BASEHELPER::GetEERegistryKeySTRING
 ...
 ***************************************************************************************/
/* public */
DWORD BASEHELPER::GetEERegistryKeyDWORD( char *regKeyName )
{
	DWORD type;
    DWORD size;
	DWORD retValue = -1;
...
} // BASEHELPER::GetEERegistryKeyDWORD
