    ...
    void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
        {
            ...
            switch( hr )
            {
                ...
                default:
                    if (FAILED(hr))
                        shell->Write(L"ERROR: can't SetIP as requested,"
                                     L" reason unknown, hr=0x%08x\n", hr);
                    break;
            }
    ...
    void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
    {
        ...
        while (count-- > 0 && !shell->m_stopLooping)
        {
            ...
            if (shell->m_currentFrame != NULL)
            {
                ...
                // the next (non-PROLOG) line of MSIL.
...
#else // !_X86_
                _ASSERTE(!"Alpha - StepSingleDebuggerCommand::Do (Commands.cpp)");
#endif // _X86_
...
public:
    ...
    void Help(Shell *shell)
    {
   		ShellCommand::Help(shell);
    	shell->Write(L"[<count>]\n");
        shell->Write(L"Displays a stack trace for the current thread. If\n");
        shell->Write(L"no argument is passed, a complete stack trace is\n");
        shell->Write(L"displayed. If a count argument is provided, the\n");
        shell->Write(L"specified number of stack frames are displayed.\n");
        shell->Write(L"\n");
    }
        ...
        else
        {
            ...
            if (f == NULL)
            {
                if (shell->m_rawCurrentFrame == NULL)
                    shell->Error(L"No current managed MSIL frame.\n");
                else
...
public:
    ...
    void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
    {
        ...
        LONG result = RegCreateKeyExA(HKEY_LOCAL_MACHINE, REG_NDP_KEY,
        ...
        if (result == ERROR_SUCCESS)
        {
            ...
            result = RegQueryValueExA(key, REG_NDP_DEBUGGER_KEY,
                                      NULL, &type, NULL, &len);
        ...
        if (setIt)
        {
            ...
            result = RegSetValueExA(key, REG_NDP_DEBUGGER_KEY, NULL,
                                    REG_SZ,
...
struct DSMInfo g_DSMData[] = 
{
    ...
    {DSM_IL_NATIVE_PRINTING, L"MSILNatPrint",
     L"Offsets will be both MSIL and native-relative",
     L"Offsets will be MSIL xor native-relative",
     L"Display offsets in MSIL or native-relative, or both",
     L"              "},
...
#else // !_X86_
        _ASSERTE(!"Alpha - RegistersDebugger2Command::Do (Commands.cpp)");
#endif // _X86_
    }
...
#else // !_X86_
        _ASSERTE(!"Alpha - WriteReg (Commands.cpp)");
#endif // _X86_
        return true;
    }
        ...
        while (shell->m_targetProcess != NULL && !shell->m_stopLooping)
        {
            ...
            if (shell->m_rawCurrentFrame)
            {
                ...
                if (currentFunc == ourFunc)
                {
                    ...
                    // Obtain IP assuming JIT compiled code
                    hr = shell->m_rawCurrentFrame->QueryInterface(
...
public:
    ...
    virtual void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
    {
        ...
        cValue = (UINT)wcstol( szRange, &pCh, 0 );
        ...
        for ( iValue = 0; iValue< cValue;iValue++)
        {
            ...
            {   //ran out of arguments
                ...
                rgbValue[iValue] = (BYTE)wcstol( szValue, &pCh, 0 );
        }
...
public:
    ...
    virtual void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
    {
        ...
        hr = shell->ResolveFullyQualifiedMethodName(methodName, &pFunc);
