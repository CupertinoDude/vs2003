...
#define REG_NDP_KEY          "Software\\Microsoft\\.NETFramework\\"
#define REG_NDP_DEBUGGER_KEY "DbgManagedDebugger"

// Names of registry keys used to hold the source files path.
#define REG_DEBUGGER_KEY  REG_NDP_KEY "Samples\\CorDbg"
...
enum DebuggerShellModes
{
    ...
    DSM_MAXIMUM_MODE             = 26, // count of all modes, not a mask.
...
public:
        ...
        m_lastFuncEval(NULL), m_steppingForStartup(false)
    {
        ...
        // port: if DWORD or ULONG size changes, create a field
        // for dwThreadId
...
struct DebuggerVarInfo
{
    LPCSTR                 name;
    PCCOR_SIGNATURE        sig;
    unsigned long          varNumber;  // placement info for MSIL code
...
public:
    DebuggerCodeBreakpoint(int breakpointID, 
                           ...
                           DebuggerFunction* function, SIZE_T offset, BOOL msil,
                           DWORD threadID);
    DebuggerCodeBreakpoint(int breakpointID, 
                           DebuggerModule* module,
