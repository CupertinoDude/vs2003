                                              ...
                                              BOOL fIsOutOfBand)
{
    ...
    case EXCEPTION_DEBUG_EVENT:
        ...
        else if (event->u.Exception.ExceptionRecord.ExceptionCode == STATUS_BREAKPOINT)
        {
            ...
            while (bp != NULL)
            {
                if (!bp->m_managed && bp->MatchUnmanaged((CORDB_ADDRESS) event->u.Exception.ExceptionRecord.ExceptionAddress))
                {
...
#else // !_X86_
                    _ASSERTE(!"Alpha - DebugEvent (dShell.cpp)");
#endif // _X86_
            ...
            while (bp != NULL)
            {
                ...
                if (!bp->m_managed &&
                    (bp->m_unmanagedModuleBase ==
                     (CORDB_ADDRESS)event->u.UnloadDll.lpBaseOfDll))
                    bp->Detach();
        ...
        if (event->u.DebugString.nDebugStringLength > 0)
        {
            ...
            if (stringBuf)
            {
                ...
                hr = pProcess->ReadMemory(
                      (CORDB_ADDRESS) event->u.DebugString.lpDebugStringData,
...
#else // !_X86_
            _ASSERTE(!"Alpha - HandleUnmanagedEvent (dShell.cpp)");
#endif // _X86_
...
HRESULT DebuggerShell::Init()
{
    ...
    if (FAILED(hr))
    {
        ...
        Write(L"(The most probable cause is that mscordbi.dll is not properly "
              L"registered.)\n\n");
...
HRESULT DebuggerShell::PrintFrame(ICorDebugFrame *frame)
{
    ...
    if (FAILED(hr))
    {
        Write(L"[Unable to obtain any frame information]");
        goto LExit;
    }
...
{
    if (fgetws(buffer, maxCount, m_in))
    {
        // Get rid of the newline character
        WCHAR *ptr = wcschr(buffer, L'\n');
        if (ptr)
        {
            _ASSERTE(ptr && "Line was too long.  Up Debugger Shell's BUFFER_SIZE.");
            *ptr = L'\0';
        }
                                        ...
                                        ULONG32 rank, ULONG32 *indicies)
{
    ...
    for (unsigned int i = 0; i < rank; i++)
    {
        ...
        if (!GetIntArg(indexStart, index))
        {
            ...
            memcpy(tmpStr, indexStart, (indexEnd - indexStart)*sizeof(WCHAR));
            tmpStr[indexEnd - indexStart] = L'\0';
    ...
    ICorDebugILFrame *context, WCHAR *name,ICorDebugValue **ppiRet )
{
    ...
    if (_wcsnicmp( name, wszVarType, wcslen(wszVarType))==0)
    {
        ...
        long number = wcstol(wszVal, &wszStop, 10);
        if (fVar == true)
                ...
                if (objContents != NULL)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            DumpMemory(objContents, (CORDB_ADDRESS)objContents, objSize, 4, 4, FALSE);
                        }
...
HRESULT DebuggerShell::ResolveFullyQualifiedMethodName(WCHAR *methodName, ICorDebugFunction **ppFunc)
{
    HRESULT hr = S_OK;
    *ppFunc = NULL;
...
#error "Platform NYI"
    ...
    hr = m_targetProcess->ReadMemory((CORDB_ADDRESS)readAddr,
                                     PAGE_SIZE,
                                     pages,
                                     &read);
    ...
    hr = m_targetProcess->ReadMemory((CORDB_ADDRESS)(readAddr + PAGE_SIZE),
                                     PAGE_SIZE,
                                     pages + PAGE_SIZE,
                                     &read);
    ...
    while (b != NULL)
    {
        if ((b->m_managed == false) &&
            ...
            (b->m_address <  (CORDB_ADDRESS)(readAddr + PAGE_SIZE * 2)))
        {
                                            ...
                                            DWORD        startAddr)
{
    ...
    else
    {
        ...
        {
            Write(L" ");
            address = function->Disassemble(!isIL,
                                            address,
                                            codeStart,
                                            codeEnd,
                                            buffer,
...
#else // !_X86_
    _ASSERTE(!"Alpha - TraceUnmanagedThreadStack (dShell.cpp)");
#endif // _X86_
}
                                        ...
                                        CORDB_ADDRESS bpEnd)
{
    ...
    while ((f.AddrFrame.Offset <= bpEnd) && (m_unmanagedDebuggingEnabled) && (i++ < 200));
}
                                          ...
                                          bool fSilently)
{
    ...
    else
    {
             ...
             m = (DebuggerModule*) g_pShell->m_modules.FindNext(&find))
        {
            ...
            char        rcFileNoExt[MAX_PATH];
            char        rcFileExt[MAX_PATH];
            char        rcExt[64];
                              ...
                              WCHAR *szModuleName)
{
    ...
    if (szModuleName)
    {
        ...
        {
            // Then get the cwd
            cwd = _wgetcwd(cwdBuffer, MAX_PATH);

            _ASSERTE(cwd != NULL); // This shouldn't fail
        ...
        *classEnd = 0;
        ...
        if (SUCCEEDED(hr))
        {
            ...
            while (TRUE)
            {
                ...
                //
                // Assume offset is MSIL only for functions which have 
                // no native code.
                // 
                ...
                if (SUCCEEDED(function->LoadCode(!msil)))
                {
                    //
                    ...
                    if (!function->ValidateInstruction(!msil, m_index))
                        continue;
                }

                DebuggerCodeBreakpoint *bp = 
                    new DebuggerCodeBreakpoint(m_id, module, 
                                               function, m_index, msil,
                                               NULL_THREAD_ID);
