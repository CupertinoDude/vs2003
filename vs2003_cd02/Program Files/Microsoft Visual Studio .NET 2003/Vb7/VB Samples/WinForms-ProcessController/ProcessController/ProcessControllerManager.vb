...
Namespace WindowsApplication1
    ...
    Public Class ProcessControllerManager
        ...
        Private Sub ShowProcessInfo(ByVal Sender As Object, ByVal e As EventArgs)
            ...
            If Key <> "" Then
                ...
                Try
                    ...
                    End Try
                    ListPcs.Items.Add(("ID: " + CStr(TempProcess.Id)))
        ...
        Private Sub LoadProcesses()
            ...
            Dim lastException As Type

            'Enable the RaisingEvents for each process.
            Dim TempProcess2 As Process = Nothing
            For Each TempProcess2 In Processes.Values
                Try
                    TempProcess2.EnableRaisingEvents = True
                Catch ex As Exception
                    If (Not (ex.GetType() Is lastException)) Then
                        If Not TempProcess2.Id = 0 Then 'this is the "idle" process - access to this is denied - will ignore
                            MessageBox.Show("Unable to register process events to remote machine '" + MachineName + "'.  " + ex.Message)
                            ' Throw New Exception(ex.Message)
                            lastException = ex.GetType()
                        End If
                    End If
                End Try
            Next TempProcess2
