...
namespace StringWalker
{
    ...
    private void buttonWalk_Click(object sender, System.EventArgs e)
    {
      ...
      bool fcontinue = sw.GetFirst(out str);
      while(fcontinue)
      {
        ...
        foreach(char c in str)
        {
          ...
          switch(uc)
          {
            case UnicodeCategory.Surrogate:
              sb.AppendFormat(rm.GetString("surrogate"));
              break;

            case UnicodeCategory.NonSpacingMark:
              sb.AppendFormat(rm.GetString("nonspacingmark"));
              break;
    ...
    private void buttonInsert_Click(object sender, System.EventArgs e)
    {
      try
      {
        int pos = Int32.Parse(InsertPos.Text);
        StringWalker sw = new StringWalker(BaseString.Text);

        if(sw.Insert(pos, InsertString.Text))
          BaseString.Text = sw.ToString();
        else
          MessageBox.Show(this, rm.GetString("err-insert1"));
      }
      catch
      {
        MessageBox.Show(this, rm.GetString("err-insert2"));
      }
    }
    ...
    private void buttonRemove_Click(object sender, System.EventArgs e)
    {
      try
      {
        int pos   = Int32.Parse(RemovePos.Text);
        int count = Int32.Parse(RemoveCount.Text);
        StringWalker sw = new StringWalker(BaseString.Text);

        if(sw.Remove(pos, count))
          BaseString.Text = sw.ToString();
        else
          MessageBox.Show(this, rm.GetString("err-remove1"));
      }
      catch
      {
        MessageBox.Show(this, rm.GetString("err-remove2"));
      }
    }
    ...
    private void buttonFind_Click(object sender, System.EventArgs e)
    {
      try
      {
        int pos   = Int32.Parse(FindPos.Text);
        StringWalker sw = new StringWalker(BaseString.Text);

        int foundPos = sw.IndexOf(FindString.Text, pos);
        if(-1 == foundPos)
          labelFind.Text = rm.GetString("notfound");
        else
          labelFind.Text = String.Format(rm.GetString("foundformat"), FindString.Text, foundPos);
      }
      catch
      {
        MessageBox.Show(this, rm.GetString("err-find"));
      }
    }
    ...
    {
      StringWalker sw = new StringWalker(BaseString.Text);
      labelTEL.Text = sw.Length.ToString();
      labelCL.Text  = BaseString.Text.Length.ToString();
    }
	}
}
