...
<script language="JScript" runat="server">
    ...
    function NewAuthorBtn_Click(sender:Object, e:EventArgs) : void {
        ...
        var authorsFile:String = Environment.GetEnvironmentVariable("TEMP") + "\\authorsJS.xml";
        if (!File.Exists(authorsFile)) {
            File.Copy(Server.MapPath("authors.xml"), authorsFile);
        }

        // open the file and read the current data
        var fs:FileStream = new FileStream(authorsFile,FileMode.Open, FileAccess.Read,
            FileShare.ReadWrite);
        ...
        // append a row
        try {
            var newAuthor:DataRow = ds.Tables[0].NewRow();
        ...
        fs = new FileStream(authorsFile, FileMode.Create, 
            FileAccess.ReadWrite, FileShare.ReadWrite);
        var writer:TextWriter = new StreamWriter(fs);
    ...
    function RefreshBtn_Click(sender:Object, e:EventArgs) : void {
    ...
    function LoadData() : void {
        ...
        var Source:DataView = DataView(Cache["MyData"]);
        ...
        if(Source == null) {
            ...
            // check to see if the file exists in the temp dir first since the ASPNET
            // process does not have permissions to write in the current directory
            var authorsFile:String = Environment.GetEnvironmentVariable("TEMP") +
                "\\authorsJS.xml";
            if (!File.Exists(authorsFile)) {
                File.Copy(Server.MapPath("authors.xml"), authorsFile);
            }
