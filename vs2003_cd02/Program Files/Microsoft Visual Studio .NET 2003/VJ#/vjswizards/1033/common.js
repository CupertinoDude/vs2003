...
var GUID_Deployment_TemplatePath          = "{96381F8D-BBFF-4037-B794-56E213A48B0E}";
...
/*****************************************************************************
  The following section contains functions that are used by JSharp Projects
  and JSharp Additems. If you like to add a new function that is JSharp
...
/******************************************************************************
     Description: Creates a J# project
  strProjectName: Project Name
  strProjectPath: The path that the project will be created in
 strTemplateFile: Project template file e.g. "default.vjsproj"
******************************************************************************/
function CreateBJProject(strProjectName, strProjectPath, strTemplateFile)
{
	try
	{
		...
		strProjectNameWithExt = strProjectName + ".vjsproj";
...
/******************************************************************************
 Description: 
       oProj: Project object
******************************************************************************/
function GetBJReferenceManager(oProj)
{
...
/******************************************************************************
 Description: 
    refmanager: Project References object
******************************************************************************/
function AddDefaultReferences(refmanager)
{
	refmanager.Add("vjslib");
}
...
function AddReferencesForClass(oProj)
{
	...
	AddDefaultReferences(refmanager);
...
******************************************************************************/
function AddFilesToBJProject(oProj, strProjectName, strProjectPath, InfFile, AddItemFile)
{
	try
	{
		dte.SuppressUI = false;
		...
		while (!strTextStream.AtEndOfStream)
		{
			...
			if (strTpl != "")
			{
				...
				if(!AddItemFile)
				{
					strTarget = GetBJTargetName(strName, strProjectName);
				}
				else
				{
				...
				// Add the extention to strTpl and to strTarget
				var extn = GetDotExtn();
				var idx = strTpl.indexOf(".");
				if (idx == -1)
					strTpl += extn;
...
strDesignerFile: Designer file name
    AddItemFile: Wether the wizard is invoked from the Add Item Dialog or not
******************************************************************************/
function AddDesignerFileToBJWebProject(oProj, strProjectName, strProjectPath, strDesignerFile, AddItemFile)
{
	dte.SuppressUI = false;
	var projItems;
	if(AddItemFile)
...
/******************************************************************************
 Description: Validate the value of the wizard combo control as a JSharp type.
     oObject: The wizard editable combo control
******************************************************************************/
function ValidateWizComboBJType(oObject, strName)
{
	var bValid;
	...
	{
		L_ValidateBJType_E_EMPTY_TEXT = " cannot be empty.";
		window.external.ReportError(strName + L_ValidateBJType_E_EMPTY_TEXT);
		bValid = false;
	}
	else if ( !window.external.ValidateCLRIdentifier(oObject.value) )
	{
		L_ValidateBJType_E_INVALID_TEXT = "Invalid ";
		window.external.ReportError(L_ValidateBJType_E_INVALID_TEXT + strName + ".");	
		bValid = false;
	}
	else
		bValid = true;
...
/******************************************************************************
 Description: Validate the value of the control as a valid JSharp name.
     oObject: The reference to control
     strName: Control name used by message
******************************************************************************/
function ValidateBJName(oObject, strName)
{
	var bValid;
	if(typeof(strName) == "undefined")
		strName = oObject.id;
	...
	if(""==oObject.value)
	{
		L_ValidateBJName_E_EMPTY_TEXT = " cannot be empty.";
		window.external.ReportError(strName + L_ValidateBJName_E_EMPTY_TEXT);
		bValid = false;
	}
	else if ( !window.external.ValidateCLRIdentifier(oObject.value) )
	{
		L_ValidateBJName_E_INVALID_TEXT = "Invalid ";
		window.external.ReportError(L_ValidateBJName_E_INVALID_TEXT + strName + ".");	
		bValid = false;
	}
	else
		bValid = true;
...
function SetTargetFullPath(oObject)
{
	...
	wizard.AddSymbol("SAFE_NAMESPACE_NAME",  strNameSpace);
    if (strNameSpace == "")
        wizard.AddSymbol("IS_NAMESPACE_NAME", false);
    else
        wizard.AddSymbol("IS_NAMESPACE_NAME", true);
...
/******************************************************************************
 Description: Returns the currently used extention
******************************************************************************/
function GetDotExtn()
{
	return ".jsl";
